...
public class CoderButtonAction {
    ...
    private void coder_action(ActionEvent e, JTextField t, JPanel p, JPanel p2, JFrame f, JButton button_reset) {
        ...
        ArrayList<ArrayList<String>> test = new ArrayList(matchArrayGenerator(t));
        ...
        for (int i = 0; i < test.get(0).size(); i++) {
            ...
            if(test.get(0).get(i).matches(regexLetter)){
                button_generate[i] = new JButton(test.get(1).get(i));
            ...
            if(!test.get(0).get(i).matches(regexSpecial)){
                button_generate[i] = new JButton(test.get(0).get(i));
                button_generate[i].setPreferredSize(new Dimension(45, 45));
            ...
            if(test.get(0).get(i).matches(regexSpace)){
                button_generate[i] = new JButton(test.get(0).get(i));
